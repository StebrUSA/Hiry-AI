...
const SkillTable = () => {
    ...
    const handleCoreSkills = (selectedOptions) => {
        ...
        const newData = selectedCoreSkills.filter((item) => selectedSkills.includes(item.skills));
        // Add rows for newly selected skills
        selectedOptions.forEach((option) => {
            const isExisting = selectedCoreSkills.some((item) => item.skills === option.label);
            if (!isExisting) {
                                    ...
                                    <tr key={index}>
                                        ...
                                        <td>
                                            ...
                                            <Input
                                                ...
                                                onChange={(e) => handleExperienceInputChange(index, e.target.value)}
                                            />
                                        </td>
                                        <td>
                                            <div className="input-step step-info">
                                                <button
                                                    type="button"
                                                    className="minus"
                                                    onClick={() => handleRatingUpdate(index, item.rating - 1)}
                                                >
                                                    –
                                                </button>
                                                <Input
